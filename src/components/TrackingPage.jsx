...
const STATUS_TIMELINE = [
    ...
    { status: 'Preparing Order', icon: Package, description: 'Preparing materials for weaving.' },
    { status: 'Under Production', icon: Wrench, description: 'Your items are being made.' },
            ...
            <div className="max-w-3xl mx-auto space-y-6">
                ...
                >
                    ...
                    <div className="p-8 md:p-10 border-b border-gray-50 bg-gradient-to-br from-white to-gray-50/50">
                        ...
                        {/* Logistics Transparency: Smart Arrival Estimation */}
                        {(order.status === 'Order Placed' || order.status === 'Payment Confirmed' || order.status === 'Under Production') ? (
                            (() => {
                                const preorderItems = (order.items || []).filter(i => i.isPreorder);
                                if (preorderItems.length > 0 || order.targetDate) {
                                    const maxDays = Math.max(...preorderItems.map(i => i.preorderDays || 14));
                                    const orderDate = order.createdAt?.toDate() || new Date();
                                    
                                    let estDate;
                                    let isCustom = false;
                                    if (order.targetDate) {
                                        estDate = new Date(order.targetDate);
                                        isCustom = true;
                                    } else {
                                        estDate = new Date(orderDate);
                                    ...
                                    return (
                                        <motion.div 
                                            ...
                                            className="bg-amber-50 p-6 rounded-[32px] border border-amber-100 flex items-start gap-4"
                                        >
                                            ...
                                            <div>
                                                <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1">
                                                    {isCustom ? "Scheduled Delivery Date" : "Craftsmanship Lead Time"}
                                                </p>
                                                <h4 className="text-sm font-black text-gray-900 mb-1">Estimated Completion: {estDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</h4>
                                                <p className="text-[10px] text-gray-500 font-bold leading-relaxed italic">
                                                    {isCustom ? "Our master weavers have scheduled your piece for this date." : `Custom items take time to make. Your order includes pre-order items requiring up to ${maxDays} days of craftsmanship.`}
                                                </p>
                                            </div>
                                        </motion.div>
                                    );
                                    ...
                                    return (
                                        <div key={step.status} className={`flex gap-6 relative z-10 ${isCompleted ? 'opacity-100' : 'opacity-40'}`}>
                                            ...
                                            <div className="pt-0.5 min-w-0">
                                                ...
                                                {/* IN-SITE PRODUCTION PROGRESS */}
                                                {isCurrent && step.status === 'Under Production' && order.productionStage && (
                                                    <div className="mt-4 p-5 bg-gray-50 rounded-2xl border border-gray-100">
                                                        <div className="flex justify-between items-center mb-2">
                                                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Master Craftsmanship</span>
                                                            <span className="text-[10px] font-black text-amber-600">
                                                                {PRODUCTION_STAGES.find(s => s.id === order.productionStage)?.progress || 0}%
                                                            </span>
                                                        </div>
                                                        <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                                            <motion.div 
                                                                initial={{ width: 0 }}
                                                                animate={{ width: `${PRODUCTION_STAGES.find(s => s.id === order.productionStage)?.progress || 0}%` }}
                                                                className="h-full bg-amber-500 rounded-full shadow-[0_0_10px_rgba(245,158,11,0.5)]"
                                                            />
                                                        </div>
                                                        <p className="text-[10px] font-bold text-gray-500 mt-3 flex items-center gap-2 italic">
                                                            <Wrench size={10} /> Currently: {PRODUCTION_STAGES.find(s => s.id === order.productionStage)?.label}
                                                        </p>
                                                    </div>
                                                )}
                                            </div>
                ...
                <p className="text-center text-[10px] text-gray-400 font-black uppercase tracking-[4px] py-10 opacity-30">KenteHaul Official Tracking</p>
            </div>
        </div>
    );
}
