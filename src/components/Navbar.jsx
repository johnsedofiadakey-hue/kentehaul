...
import { Menu, X, ShoppingBag, ChevronDown, Truck, ArrowRight, Star, Sparkles, Phone, MapPin, Search, Heart, User } from 'lucide-react';
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
                    <div className="flex justify-between items-center h-16 md:h-18">
                        ...
                        <div className="flex items-center gap-2 md:gap-4">
                            ...
                            <button
                                onClick={() => isClientAuthenticated ? null : setIsClientAuthOpen(true)}
                                className="relative flex flex-col items-center group pt-0.5"
                            >
                                <div className={`p-3 rounded-2xl transition-all relative ${isClientAuthenticated ? 'bg-indigo-50 text-indigo-600' : 'bg-gray-50 text-gray-500 hover:bg-indigo-500 hover:text-white'}`}>
                                    <User size={20} className="group-hover:scale-110 transition-all" />
                                    {isClientAuthenticated && (
                                        <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-green-500 rounded-full border-2 border-white" />
                                    )}
                                </div>
                                <span className={`text-[9px] font-black uppercase mt-1 tracking-widest transition-colors ${isClientAuthenticated ? 'text-indigo-600' : 'text-gray-400 group-hover:text-indigo-600'}`}>
                                    {isClientAuthenticated ? (clientUser?.name?.split(' ')[0] || 'Royal') : 'Login'}
                                </span>

                                {/* Profile Dropdown (Simplified) */}
                                {isClientAuthenticated && (
                                    <div className="absolute top-16 right-0 w-48 bg-white shadow-2xl rounded-3xl border border-gray-50 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all p-2 z-[200]">
                                        <div className="px-4 py-3 border-b border-gray-50">
                                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Authenticated As</p>
                                            <p className="font-black text-gray-900 text-sm truncate">{clientUser?.name}</p>
                                        </div>
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); handleClientLogout(); }}
                                            className="w-full text-left px-4 py-3 text-red-500 hover:bg-red-50 rounded-2xl text-xs font-black uppercase tracking-widest transition-colors flex items-center gap-2"
                                        >
                                            <X size={14} /> Sign Out
                                        </button>
                                    </div>
                ...
                <AnimatePresence>
                    {mobileMenuOpen && (
                        <motion.div
                            ...
                            className="lg:hidden fixed inset-x-0 top-[72px] bottom-0 z-[100] bg-white/40 backdrop-blur-xl overflow-hidden border-t border-white/20 shadow-[-20px_0_80px_rgba(0,0,0,0.1)] pointer-events-none"
                        >
                            <div className="h-full overflow-y-auto custom-scrollbar p-6 pt-10 space-y-2 pointer-events-auto">
                                {/* MOBILE SEARCH */}
                                ...
                                <div className="space-y-3 mb-6">
                                    ...
                                    {/* MOBILE CLIENT AUTH */}
                                    <button
                                        onClick={() => { if (!isClientAuthenticated) setIsClientAuthOpen(true); else if (window.confirm("Logout?")) handleClientLogout(); closeMenus(); }}
                                        className={`w-full flex items-center justify-between p-5 rounded-[24px] active:scale-95 transition ${isClientAuthenticated ? 'bg-indigo-50 text-indigo-600' : 'bg-gray-50 text-gray-900'}`}
                                    >
                                        <div className="flex items-center gap-3">
                                            <User size={20} />
                                            <span className="font-black text-xs uppercase tracking-widest">
                                                {isClientAuthenticated ? `Logged in as ${clientUser?.name?.split(' ')[0]}` : 'Sign In / Register'}
                                            </span>
                                        </div>
                                        {isClientAuthenticated ? <X size={18} className="opacity-40" /> : <ArrowRight size={18} className="opacity-20" />}
                                    </button>
                                </div>
