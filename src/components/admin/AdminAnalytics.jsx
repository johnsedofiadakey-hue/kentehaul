...
export default function AdminAnalytics({ products, orders }) {
        ...
        const setupListeners = () => {
            try {
                unsubActivity = onSnapshot(collection(db, "activity_log"), (snap) => {
                    ...
                    setError("Failed to stream activity logs.");
                });
                ...
                unsubWishlists = onSnapshot(collection(db, "wishlists"), (snap) => {
                    ...
                    setError("Failed to stream wishlist data.");
                });
            } catch (e) {
                console.error("Listener Setup error:", e);
                setError("Critical data connection failure.");
            }
        };
    ...
    // Decouple BI calculation from the listeners to prevent nested state updates
    useEffect(() => {
        try {
            calculateBI(wishlists, activity);
        } catch (e) {
            console.error("BI Calculation Error:", e);
            // Don't set global error here to prevent total UI crash, just log it
        }
    }, [wishlists, activity, products, orders]);
    ...
    const calculateBI = (wishlistData, activityData) => {
        try {
            ...
            activityData.forEach(log => {
                // Ensure we have a valid timestamp for filtering
                const logTime = log.timestamp?.toMillis ? log.timestamp.toMillis() : (Number(log.timestamp) || 0);
            ...
            setStats(prev => ({
                ...prev,
                totalViews: activityData.filter(l => l.type === 'view_item').length,
                totalVisits: activityData.filter(l => l.type === 'site_visit').length,
                totalHearts: wishlistData.reduce((acc, curr) => acc + (curr.itemIds?.length || 0), 0),
                ...
                mostRising: sortedRising.length > 0 ? sortedRising.slice(0, 10) : sortedViews.slice(0, 10), // Fallback to viewed if no rising data
                mostPotential: sortedPotential,
                history: historyList.slice(0, 30),
                recentActivity: wishlistData.sort((a, b) => {
                    const timeA = a.updatedAt?.toMillis ? a.updatedAt.toMillis() : 0;
                    const timeB = b.updatedAt?.toMillis ? b.updatedAt.toMillis() : 0;
                    return timeB - timeA;
                }).slice(0, 10),
                viewTrend: calculateTrend(historyList, 'views'),
                heartTrend: calculateTrend(historyList, 'hearts')
            }));
        } catch (error) {
            console.error("BI Calculation Fatal Error:", error);
        }
    };

    const calculateTrend = (history, key) => {
        if (!history || history.length < 2) return 0;
        const current = history[0][key] || 0;
        const previous = history[1][key] || 0;
    ...
    if (error) return (
        <div className="p-20 text-center bg-red-50 rounded-[40px] border border-red-100 animate-fade-in">
            <div className="w-16 h-16 bg-red-500 text-white rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg">
                <Activity size={32} />
            </div>
            <h2 className="text-2xl font-black text-gray-900 mb-2 uppercase tracking-tight">Intelligence Offline</h2>
            <p className="text-red-600 font-bold text-sm">{error}</p>
            <button onClick={() => window.location.reload()} className="mt-8 px-8 py-3 bg-gray-900 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-black transition-all">Re-Synchronize</button>
        </div>
    );
    ...
    if (loading) return (
        <div className="space-y-10 animate-pulse pb-24">
            <div className="flex justify-between items-center">
                ...
                <div className="h-12 w-80 bg-gray-100 rounded-2xl" />
            </div>
            <div className="grid grid-cols-4 gap-6">
                {[1,2,3,4].map(i => <div key={i} className="h-48 bg-gray-50 rounded-[40px] border border-gray-100" />)}
            </div>
            <div className="grid grid-cols-2 gap-8">
                <div className="h-80 bg-gray-50 rounded-[50px]" />
                <div className="h-80 bg-gray-50 rounded-[50px]" />
            </div>
        </div>
    );
    ...
    return (
        <div className="space-y-10 animate-fade-in-up pb-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title="Total Impressions" value={stats.totalViews} icon={Eye} color="bg-blue-500" trend={stats.viewTrend} />
                <StatCard title="Active Desires" value={stats.totalHearts} icon={Heart} color="bg-red-500" trend={stats.heartTrend} />
                                    ...
                                    {(stats.history || []).map((day, idx) => (
                                        <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="py-5 px-4 text-right">
                                                {(() => {
                                                    const prevDay = stats.history[idx + 1];
                                                    const growth = prevDay ? (day.visits - prevDay.visits) : 0;
                                                    if (growth > 0) return <div className="inline-flex items-center gap-1.5 text-green-500 font-extrabold text-[10px] uppercase tracking-widest"><TrendingUp size={12} /> Positive</div>;
                                                    if (growth < 0) return <div className="inline-flex items-center gap-1.5 text-amber-500 font-extrabold text-[10px] uppercase tracking-widest"><Activity size={12} /> Slow</div>;
                                                    return <div className="inline-flex items-center gap-1.5 text-gray-400 font-extrabold text-[10px] uppercase tracking-widest"><Target size={12} /> Stable</div>;
                                                })()}
                                            </td>
                                        </tr>
                                    ))}
                                    ...
                                    <div>
                                        ...
                                        <div className="p-5 bg-white/5 rounded-[28px] border border-white/5 hover:bg-white/10 transition-colors">
                                            <p className="text-[11px] font-bold text-gray-300 leading-relaxed">
                                                Shopper <span className="text-white font-black">...{wl.id.slice(-6).toUpperCase()}</span> updated their wishlist. 
                                                Added <span className="text-amber-500 font-black">{wl.itemIds?.length || 0} items</span> to their wishlist.
                                            </p>
