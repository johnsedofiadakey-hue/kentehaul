...
import { Package, Plus, DollarSign, CreditCard, Edit, Printer, ChevronDown, TrendingUp, Clock } from 'lucide-react';
...
export default function AdminOrders({
    ...
    const filteredOrdersList = orders
        ...
        .sort((a, b) => {
            // Sort newest first using order ID date component if available
    ...
    const paidRevenue = orders.filter(o => ['Paid', 'Delivered'].includes(o.status)).reduce((acc, o) => acc + (Number(o.total) || 0), 0);
                        ...
                        <div key={order.id} className="border border-gray-100 p-5 md:p-8 rounded-[32px] md:rounded-[40px] bg-white shadow-sm hover:shadow-xl transition-all duration-500">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-5">
                                ...
                                <div className="flex items-start gap-4">
                                    <div className={`h-12 w-12 md:h-16 md:w-16 rounded-[20px] md:rounded-[24px] flex-shrink-0 flex items-center justify-center text-white shadow-lg ${['Paid', 'Delivered'].includes(order.status) ? 'bg-green-500' : order.status === 'Cancelled' ? 'bg-gray-300' : 'bg-yellow-400'}`}>
                                        <Package size={22} />
                                    </div>
                                    <div>
