...
export default function InvoiceModal({ isOpen, onClose, order, siteContent }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-xl shadow-2xl flex flex-col overflow-hidden animate-fade-in">
        
        {/* --- ACTIONS HEADER --- */}
        <div className="bg-gray-900 text-white p-4 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-4">
            <h2 className="font-bold text-lg hidden md:block">Document Viewer</h2>
            <span className={`px-3 py-1 rounded-full text-xs font-bold ${isPaid ? 'bg-green-500 text-black' : 'bg-yellow-500 text-black'}`}>
              {order.status.toUpperCase()}
            </span>
          </div>
          <div className="flex gap-3">
            <button onClick={handlePrint} className="flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg transition text-sm font-bold">
        ...
        <div className="flex-1 overflow-y-auto bg-gray-100 p-4 md:p-8">
          <div ref={componentRef} className="bg-white max-w-3xl mx-auto min-h-[1000px] shadow-lg p-8 md:p-12 text-gray-800 relative">
            
            {/* 1. HEADER */}
            <div className="flex justify-between items-start border-b-2 border-gray-100 pb-8 mb-8">
              <div>
                 {siteContent.logo ? (
                   <img src={siteContent.logo} alt="Logo" className="h-24 w-auto object-contain mb-4" />
                 ) : (
                   <h1 className="text-4xl font-bold tracking-tight text-gray-900">{siteContent.heroTitle || "KenteHaul"}</h1>
                 )}
              ...
              <div className="text-right">
                <h1 className="text-5xl font-black tracking-widest opacity-10" style={{ color: docColor }}>{docType}</h1>
            ...
            <div className="flex flex-col md:flex-row gap-12 mb-12">
              <div className="flex-1">
                ...
                <div className="border-l-4 pl-4" style={{ borderColor: docColor }}>
                  <p className="font-bold text-xl text-gray-900">{order.customer?.name || "Guest Customer"}</p>
            ...
            <div className="mb-12">
              <table className="w-full text-left">
                <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
                  <tr>
                    <th className="p-4 rounded-l-lg">Description</th>
                  ...
                  {order.items.map((item, index) => (
                    <tr key={index}>
                      <td className="p-4 font-medium text-gray-900">
                        {item.name}
                        {item.subcategory && <span className="block text-xs text-gray-400 font-normal">{item.subcategory}</span>}
                      </td>
                      <td className="p-4 text-center font-mono">{item.quantity}</td>
                      <td className="p-4 text-right font-mono">₵{item.price}</td>
                      <td className="p-4 text-right font-bold font-mono">₵{(item.price * item.quantity).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex justify-end mb-12">
               <div className="w-64 space-y-3">
                 ...
                 <div className="border-t pt-3 flex justify-between items-center">
                   ...
                   <span className="font-black text-2xl" style={{ color: docColor }}>₵{order.total}</span>
                 </div>
               </div>
            </div>

            {/* 5. FOOTER / BANK DETAILS */}
            <div className="border-t-2 border-gray-100 pt-8 flex flex-col md:flex-row gap-8 text-sm">
              <div className="flex-1">
            ...
            <div className="mt-16 text-center">
               <p className="text-xs text-gray-300 font-bold uppercase tracking-widest">Generated by KenteHaul Admin System</p>
            </div>
